...
\begin{table}[htbp]
  ...
  \caption{Reference extinction times and DR-PINN threshold-crossing
    ...
    before $T$ in most runs; where it is, the reported value lies so close
    to $T$ that persistence is certified over a window of only a few
    evaluation steps, so these entries should be regarded as inconclusive
    rather than as evidence of persistent extinction.  The evaluation-grid
    step is $2.5\times10^{-3}$, corresponding to a resolution uncertainty
    of about $1.3\times10^{-3}$.  Threshold crossings are therefore not
    identified with exact finite-time extinction of the smooth network
    output.}
...
the monotone ordering of the reference extinction times.  Moreover,
...
an extinction time.  We also note that, with only three seeds per value
of $\lambda$, the reported ranges are indicative rather than
statistically robust; a larger seed ensemble would be needed to
characterize the seed-to-seed variability quantitatively.
...
preserve the monotone ordering of the reference extinction times,
...
underlying Figures~9--12; the exact definitions and weights of all
