...
%% --------------------------------------------------------
%%  Auto-generated numerical results (single-run synchronization)
%% --------------------------------------------------------
%% The replication scripts code/src/run_experiment_6{1,2,3}.py write
%% generated/results_6{1,2,3}.tex in a single documented run each, together
%% with all figures and a JSON manifest. Every experiment-dependent number
%% quoted in Section 6 is a macro defined in those files, so the text, the
%% tables and the figures provably originate from the same run. If a
%% generated file is missing, the \providecommand fallbacks below render a
%% visible red TBD marker instead of silently keeping a stale value.
...
%%% Reference-solver extinction times are deterministic and have been
%%% independently verified (operator splitting, N=49, dt=5e-4); the trained
%%% network quantities are TBD until the replication script has been run.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{cccc}
    ...
    \midrule
    0.4 & \RelayTRefA & \RelayTNetA & \RelayRelErrA \\
    0.6 & \RelayTRefB & \RelayTNetB & \RelayRelErrB \\
    0.8 & \RelayTRefC & \RelayTNetC & \RelayRelErrC \\
    \bottomrule
  \end{tabular}
  \caption{Reference and DR-PINN extinction times $t^*$ for the primary sweep
    ...
    since the network is evaluated on a time grid of step
    $2.5\times10^{-3}$, the detected DR-PINN times carry a resolution
    uncertainty of about half the evaluation step
    ($\approx1.3\times10^{-3}$), so the trailing digits should not be
    over-interpreted.}
...
$\approx\RelayZFrontMax$. This experiment therefore demonstrates an accurate
\emph{qualitative} reproduction of the finite-time extinction dynamics
and of the extinction times, but it does not exhibit a uniformly small
distance residual over the whole domain, and should not be read as a
direct numerical realization of the hypothesis
$\mathcal{J}(u_n)\to0$ of Theorem~\ref{thm:consistency-parabolic}.
...
so a small residual plateau after extinction is expected. The right panel shows the pointwise absolute error
...
$t^*(0.4)>t^*(0.6)>t^*(0.8)$. The absolute errors range from
$\RelayAbsErrMin$ to
$\RelayAbsErrMax$ time units. Note that the effective loss scale grows as
$\lambda^2$ (the
distance-residual target $z\to\pm\lambda$ grows with $\lambda$), so a
fixed training budget is expected to resolve the extinction front with
somewhat lower absolute accuracy at larger $\lambda$.
...
collocation losses of order $10^{-14}$ with endpoint constraints enforced
...
inclusion with relay feedback, the DR-PINN reproduces the finite-time
extinction phenomenon and the correct ordering of extinction times across
$\lambda$, while the branch-selection diagnostic makes visible both the
learned selection mechanism and the concentration of the residual along
the extinction front, where a smooth surrogate must approximate a
