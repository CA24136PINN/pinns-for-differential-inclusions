...
\begin{proof}
	...
	\qquad s\in\mathbb{R},
	\]
	standard existence theory for semilinear parabolic equations yields a
	strong solution with exactly the parabolic maximal $L^2$-regularity
	stated below: the linear Dirichlet heat equation with right-hand side in
	$L^2(Q)$ and initial datum in $H_0^1(\Omega)$ admits a unique solution in
	$L^2(0,T;H^2(\Omega)\cap H_0^1(\Omega))\cap H^1(0,T;L^2(\Omega))$
	(see \cite[Section~7.1.3, Theorem~5]{Evans} for smooth domains and
	\cite[Theorem~3.2.1.2]{Grisvard1985} for the $H^2$ elliptic regularity on
	convex or $C^{1,1}$ domains), and a Galerkin compactness argument or a
	Schauder fixed-point argument extends this to the semilinear problem
	under the continuity and linear growth conditions on $f$ (a Banach
	fixed-point argument would additionally require a Lipschitz condition
	on $f$, which we do not assume); see also
	\cite[Chapter~4]{LionsMagenes}. This
	provides
	\[
	u\in
	L^2\!\bigl(0,T;H^2(\Omega)\cap H_0^1(\Omega)\bigr)
...
\end{remark}
	...
	\begin{itemize}
		...
		\item[(H2)] $t\mapsto F(t,x)$ is (Lebesgue) measurable for \emph{every}
		$x\in\mathbb{R}^d$, and $x\mapsto F(t,x)$ is upper semicontinuous for
		\emph{a.e.} $t\in[0,T]$.
	...
	\begin{lemma}\label{lem:normal-integrand}
		...
		\begin{enumerate}
			...
			\item $g(\cdot, x, v)$ is measurable in $t$ for every $(x,v)\in\mathbb{R}^d\times\mathbb{R}^d$.
		\end{enumerate}
	\end{lemma}
	\begin{proof}
		...
		Finally, we prove $(c)$. Fix $(x,v)\in\mathbb{R}^d\times\mathbb{R}^d$.
		By hypothesis (H2), the multifunction $t\mapsto F(t,x)$ is measurable
		with closed (indeed compact) values, and the distance function to a
		measurable closed-valued multifunction is measurable in $t$ for every
		fixed $v$ (see, e.g., \cite[Chapter~8]{aubin_frankowska_2009}). Hence
		$t\mapsto g(t,x,v)=\dist^2(v,F(t,x))$ is measurable, which proves
		$(c)$.
	\end{proof}
	...
	\begin{proof}
		...
		\]
		Moreover, $t\mapsto w_n(t)$ is (Lebesgue) measurable. First, by \textup{(H1)}--\textup{(H2)}, $F$ is a Carath\'eodory-type multifunction: measurable in $t$ for every $x$, upper semicontinuous in $x$ for a.e.\ $t$, with compact values. Such multifunctions are superpositionally measurable, i.e., $t\mapsto F(t,y(t))$ is measurable for every measurable (in particular, continuous) function $y$; see, e.g., \cite[Chapter~8]{aubin_frankowska_2009}. Applied with $y=x_n$, this shows that the composed multifunction $t\mapsto F(t,x_n(t))$ is measurable with compact convex values. Only then does the measurable-projection theorem apply: the metric projection of the measurable function $\dot{x}_n$ onto a measurable, closed- and convex-valued multifunction is measurable; see \cite[Theorem~8.2.11]{aubin_frankowska_2009}.
...
\end{proof}
	...
	\begin{proof}
		...
		\bigl(z_n(t,x)\bigr).
		\]
		The map $w_n$ is (Lebesgue) measurable on $Q$: by \textup{(P1)}--\textup{(P2)} the multifunction $\Phi$ is measurable with closed convex values, hence so is $(t,x)\mapsto\Phi(u_n(t,x))$ as the composition with the measurable function $u_n$, and the metric projection of the measurable function $z_n$ onto it is measurable by the measurable-projection theorem for measurable, closed- and convex-valued multifunctions; see \cite[Theorem~8.2.11]{aubin_frankowska_2009}. It satisfies
		\[
		...
		normal integrands that are convex in the second variable
		(\cite[Theorem~1]{berkovitz_1974}; see also \cite{ioffe_1977} and
		\cite[Chapter~VIII]{ekeland_temam_1999}), applied with
		$u_n\to u^\ast$ strongly in $L^2(Q)$ and
		$\partial_t u_n-\Delta u_n\rightharpoonup\partial_t u^\ast-\Delta u^\ast$
		weakly in $L^2(Q)$, we obtain
		\[
		\begin{aligned}
			\mathcal{J}(u^\ast)
			&=
...
throughout $[0,T]$, an empirical illustration of
Theorem~\ref{thm:consistency}: a vanishing inclusion loss produces a
trajectory close to empirically generated admissible ones. 
\begin{figure}[htbp]
	\centering
...
solutions holds here: since $-\Delta+\lambda\,\partial|\cdot|=\partial E$
is maximal monotone (cf.\ the remark above), standard monotonicity
arguments show that
...
candidates whose distance residual tends to zero converges, along a
subsequence, to an exact solution of the target inclusion. The proofs
