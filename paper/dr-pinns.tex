...
%% TODO [Rev.]: naprawic integracje odcinkow sterowania w kodzie referencyjnym
%% (kazdy odcinek musi byc calkowany do rzeczywistego konca odcinka
%% przelaczenia, a nie do ostatniego punktu globalnej siatki), po czym
%% ponownie wygenerowac Figure~\ref{fig:phase-linear} i przeliczyc wartosc
%% 0.0445 ponizej. Ujednolicic tez liczbe punktow siatki referencyjnej
%% (paper: 200, kod: 150).
...
reported value
\[
  \overline{d_H^+}(\mathcal{R}_\theta,\widehat{\mathcal{R}}_{\rm ref}) = 0.0445,
\]
%% TODO [Rev.]: wartosc 0.0445 przeliczyc po poprawieniu integratora
%% referencyjnego (patrz TODO powyzej). Recenzent zweryfikowal niezaleznie,
%% ze roznice trajektorii stary-vs-poprawny integrator sa rzedu 0.05 (max)
%% i 0.011 (srednia), czyli tego samego rzedu co 0.0445 -- przeliczenie
%% jest wiec OBOWIAZKOWE, stara wartosc nie moze zostac.
...
%% TODO [Rev.]: dodac niezalezna walidacje residualu na gestej siatce
%% czasowej (mean i max inclusion distance), analogicznie do eksperymentu
%% z elipsa w Sec. 6.2 -- loss 1.2e-12 na 300 punktach kolokacyjnych nie
%% gwarantuje malego residualu miedzy punktami. Po przeliczeniu wstawic tu
%% jedno zdanie z wynikami.
...
Quickhull reduction is performed \emph{offline} and does not contribute to
the per-epoch time, when $\widetilde{K}=K_{\mathrm{raw}}$ the two variants
solve identical QPs, and the residual differences between their timings
are within run-to-run measurement variability rather than a preprocessing
cost.
%% TODO [Rev.]: powtorzyc badanie czasow w jednym spojnym uruchomieniu
%% generujacym tabele, tekst i wykres; kilka powtorzen na punkt, raportowac
%% srednia +/- odchylenie, dodac warm-up przed pomiarem oraz opis CPU/GPU i
%% wersji bibliotek. Czasy w ostatnim notebooku roznia sie od czasow w
%% tekscie -- zsynchronizowac.
...
at every evaluation point of every candidate trajectory and the remaining
...
In the implementation the branch selection is carried out with a numerical
tolerance $\varepsilon=10^{-6}$: the interval branch $[-\lambda,\lambda]$
is applied whenever $|s|\le\varepsilon$, i.e., the code works with the
regularized multifunction
\[
\Phi_\varepsilon(s)
=
\begin{cases}
...
\end{cases}
\]
rather than with the exact $\Phi$ of \eqref{eq:relay-Phi}. Since
$\Phi_\varepsilon(s)\supseteq\Phi(s)$ for every $s$, the resulting
distance residual is a lower bound of the exact one; $\Phi_\varepsilon$
satisfies (P1)--(P3) with the same constant $m_0=\lambda$, so the
consistency theory of Section~\ref{sec:pde-consistency} applies to it
verbatim. All quantitative results for the \emph{trained network} reported
below therefore refer to $\Phi_\varepsilon$, whereas the reference solver
of the next paragraph solves the exact problem with $\Phi$ of
\eqref{eq:relay-Phi}; the two multifunctions coincide outside the thin set
$\{0<|s|\le\varepsilon\}$, so the comparison mixes two (slightly)
different problems, a limitation to be kept in mind when reading the
results.
%% TODO [Rev.]: rozstrzygnac kwestie eps przed wysylka. Opcja A (najczystsza):
%% ustawic eps=0 w kodzie, powtorzyc trening; wtedy akapit o Phi_eps mozna
%% usunac. Opcja B: zostawic trening z Phi_eps, ale po treningu raportowac
%% OBA residuale (wzgledem Phi_eps i dokladnego Phi) oraz wykonac test
%% wrazliwosci dla eps in {0, 1e-8, 1e-6, 1e-4}.
...
$\sqrt{0.39}\approx0.62$, comparable to $\lambda=0.6$ itself, and (as
...
$\approx7$. This experiment therefore demonstrates an accurate
...
the maximum absolute norm discrepancy, normalized by the maximum reference
norm, is $0.76\%$ over $[0,T]$.
After the reference solution extinguishes at $t^*_{\rm ref}=0.1645$ (dropping
to machine zero), the DR-PINN norm stabilizes at $\approx10^{-4}$: a
...
\begin{figure}[htbp]
  ...
  \caption{Branch-selection diagnostic ($\lambda=0.6$): empirical cloud of
    ...
    where the discontinuity of $\Phi$ limits pointwise accuracy.}
    %% TODO [Rev.]: wygenerowac finalna wersje wykresu z tego samego
    %% uruchomienia co tekst (obecny wykres i tekst pochodza z roznych
    %% uruchomien); jesli inset ma wrocic, dodac zdanie o insecie z powrotem
    %% do podpisu.
