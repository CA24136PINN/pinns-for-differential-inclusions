...
\begin{document}
	...
	\begin{frontmatter}
		...
		\begin{abstract}
			...
			and amenable to gradient-based training (the squared distance is
			differentiable with respect to the network parameters whenever the
			admissible set is convex, with gradient given by the metric projection).
	...
	framework is the one whose the governing law takes the \emph{set-valued} form
...
%	
	...
	\begin{definition}
		...
		\[
		\mathcal{F}(x) \cap V \neq \varnothing \quad \forall x \in V.
		\] 
		%
	...
	If $C \subseteq X$ is a closed set, then the infimum
	%
	\begin{equation*}
		\text{dist}(x_0,C) := \inf\{\|x_0 - x\|_X : x \in C\}, \quad x_0 \in X,
	\end{equation*}
	%
	is always attained. We say that $z\in C$ is a projection of $x_0$ onto $C$ whenever $\text{dist}(x_0,C)=\|z - x_0\|_X$. In general there can be several projections of $x_0$ onto $C$, that is, there might be more than one point in $C$ closest to $x_0$.
	%
	A fundamental result in convex analysis (see, e.g., \cite[Section~8.1]{boyd_vandenberghe_2004}) establishes that the projection is unique whenever $C$ is closed and convex, and the norm $\|\cdot\|_X$ is strictly convex.
	...
	absolutely continuous functions on $[0,T]\times\mathbb{R}^d$,
...
Let $\Omega\subset\mathbb{R}^d$ be a bounded domain with smooth boundary, let $T>0$, and set
...
\end{remark}
	...
	\begin{remark}
		...
		when $\mathcal{F}(u)$ is convex and $\|\cdot\|_Y$ is strictly convex, the projection
		$P_{\mathcal{F}(u)}$ on $\mathcal{F}(u)$ is uniquely defined (cf. Section~\ref{sec:projection}), and
		\[
		R(u)
		=
		...
		\nabla R(u)
		=
		2\Bigl(\mathcal{D}(u)-P_{\mathcal{F}(u)}\bigl(\mathcal{D}(u)\bigr)\Bigr),
		\]
		which is convenient for gradient-based optimization.
	\end{remark}
	...
	\begin{theorem}[General consistency]\label{thm:general-consistency}
		...
		and let $\mathcal{F}:X\rightrightarrows Y$ be strict and closed-valued.
		Let $\{u_n\}_{n\in\mathbb{N}}\subset X$ satisfy $u_n\to x$ in $X$ and
		\[
	...
	\begin{proof}
		Since $\mathcal{D}$ is continuous, $\mathcal{D}(u_n)\to\mathcal{D}(u)$. The result then follows by appling Lemma~\ref{lem:closed-graph} with with $x_n=u_n$ and $y_n=\mathcal{D}(u_n)$.
	\end{proof}
	...
	\begin{remark}
		...
		\]
		which is
		well-defined and Lipschitz --- properties that are particularly
		favorable for gradient-based training.
	\end{remark}
	...
	\begin{proof}
		...
		since $F(t,x)$ is compact, the projections onto $F(t,x)$ are unique:
		\[
		...
		\begin{equation}
			\text{dist}(v_n, F(t, x_n)) \ge \text{dist}(v, F(t, x_n)) - |v_n - v| \quad \text{ for all } k\in\mathbb{N}.
		\end{equation}
	...
	\begin{theorem}[Consistency of the distance residual] \label{thm:consistency}
		...
		Then:
		\begin{enumerate}
			\item[\rm(a)] (Exactness) For every $x \in A$, $\mathcal{J}(x) = 0$ if and only if $x \in S(x_0)$.
			\item[\rm(b)] (Asymptotic consistency) Let $\{x_n\}_{n=1}^{\infty} \subset \mathcal{A}$ with $\mathcal{J}(x_n) \to 0$. Then there exist a subsequence (not relabeled) and $x^* \in \mathcal{A}$ such that $x_n \to x^*$ uniformly on $[0,T]$, $\dot{x}_n \rightharpoonup \dot{x}^*$ weakly in $L^2(0,T; \mathbb{R}^d)$, and $x^* \in S(x_0)$.
		\end{enumerate}
	\end{theorem}
	...
	\begin{proof}
		...
		Since $W^{1,2}(0,T; \mathbb{R}^d)$ is a Hilbert space (hence reflexive), 
		by \cite[Theorem 3.18]{brezis_2011}, there exists a subsequence such that
		\[
		x_n \rightharpoonup x^* \quad \text{weakly in } W^{1,2}(0,T; \mathbb{R}^d).
		\]
		This weak convergence in $W^{1,2}(0,T;\mathbb{R}^d)$ implies $\dot{x}_n \rightharpoonup w$ weakly in $L^2(0,T; \mathbb{R}^d)$, because the weak limit is determined by the bilinear form, and strong convergence of $x_n$, by Rellich–Kondrachov Theorem, forces the derivative component to converge weakly.
		%
		...
		\]
		Then, passing to the limit in $x_n(t) = x_0 + \int_0^t \dot{x}_n(s) \, ds$ gives 
		\[
		x^*(t) = x_0 + \int_0^t w(s) \, ds
		\]
		hence $x^* \in \mathcal{A}$ and $\dot{x}^* = w$ a.e.
		
		Finally, set $g(t, x, v) := \operatorname{dist}^2(v, F(t,x))$. 
		By Lemma~\ref{lem:normal-integrand}, $g$ is a normal convex integrand. The classical lower semicontinuity theorem for integral functionals with normal convex integrands (\cite[Theorem~1]{berkovitz_1974}; see also Ioffe [13] and [14, Chapter VIII]), applied with $x_n \to x^*$ uniformly and $\dot{x}_n \rightharpoonup \dot{x}^*$ weakly in $L^2(0,T;\mathbb{R}^d)$, yields
		%
		\[
		\mathcal{J}(x^*) \le \liminf_{n \to \infty} \mathcal{J}(x_n) = 0.
		\]
		%
		As $\mathcal{J}$ is non-negative, we conclude that $\mathcal{J}(x^*) = 0$. By part (a), this means that $x^* \in S(x_0)$.
	\end{proof}
...
\end{proof}
	...
	\begin{proof}
		...
		$u_0\in H_0^1(\Omega)$ and $\partial\Omega$ is smooth ($C^{1,1}$ regularity is enough), there exists a constant
...
The hard-admissible parametrization above does not, by itself, exercise the
distance-residual loss. To verify that the DR-PINN approach of
Section~\ref{sec:description-formulation} also applies directly to this
benchmark --- and to quantify what the hard parametrization buys --- we
additionally trained a neural trajectory
$x_\theta(t)=x_0+t\,N_\theta(t)$ (hard initial condition; $N_\theta$ a
$3\times64$ tanh network) with the genuine distance-residual loss
\begin{equation}
	\label{eq:ellipse-drpinn-loss}
	\mathcal{L}(\theta)
	=\frac{1}{N}\sum_{i=1}^{N}
	\dist^2\!\bigl(\dot{x}_\theta(t_i)-g(t_i,x_\theta(t_i)),\,E(t_i)\bigr)
	+\lambda_{\rm f}\,\bigl|x_\theta(T)-x_{\rm f}\bigr|^2,
\end{equation}
with $N=512$ uniform collocation points and $\lambda_{\rm f}=10$. Since the
projection onto an ellipse has no closed form, $\dist^2(v,E(t))$ is
evaluated pointwise by Newton iteration on the secular equation
$\sum_{k}\bigl(\sigma_k\,v^{\rm loc}_k/(\sigma_k^2+\mu)\bigr)^2=1$ in the
local frame of $E(t)$ (with $\sigma_1=a(t)$, $\sigma_2=b(t)$); the multiplier
$\mu$ is treated as a constant in the backward pass, which yields the exact
gradient $2\bigl(v-\Pi_{E(t)}(v)\bigr)$ by the projection-gradient identity
of Section~\ref{sec:description-formulation}. This illustrates that the
DR-PINN framework accommodates admissible sets whose projection is only
\emph{numerically} available. After $9000$ Adam epochs (hold-then-decay
learning rate, double precision), the DR-PINN steers the state to the target
with $|x_\theta(T)-x_{\rm f}|=1.3\times10^{-4}$, while the inclusion
residual $\dist(\dot{x}_\theta-g,E(t))$ attains a mean of
$3.9\times10^{-2}$ and a maximum of $0.14$ over a dense evaluation grid:
because the target lies close to the boundary of the reachable set,
near-extremal velocities are required for most of the horizon, and the soft
distance penalty settles for velocities slightly \emph{outside} $E(t)$
(mean level $1.19$). The comparison with
Table~\ref{tab:ellipse-continuation} makes the trade-off quantitative: the
distance-residual route recovers the correct steering behavior with a small
residual infeasibility, whereas the hard-admissible parametrization
guarantees $\ell<1$ exactly --- which is why we adopt the latter for this
example, and why hard constructions are preferable whenever the admissible
set carries an explicit parametric description.
...
with the same initial datum, so $S(u_0)$ is a singleton. A single trained
