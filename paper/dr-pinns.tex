...
\begin{proof}
	...
	convex or $C^{1,1}$ domains), and a Banach fixed-point (or Galerkin)
	argument extends this to the semilinear problem under the linear growth
	condition on $f$; see also \cite[Chapter~4]{LionsMagenes}. This
	provides
	\[
	u\in
	L^2\!\bigl(0,T;H^2(\Omega)\cap H_0^1(\Omega)\bigr)
...
\end{remark}
	...
	\begin{definition}[Distance residual]
		...
		\]
		%
		In the concrete settings considered below, $\mathcal{D}u$ and
		$\mathcal{F}u$ are functions of a variable $z$ ranging over a time
		interval or a space-time cylinder, and the inclusion
		\eqref{eq:abstract-inclusion} is imposed pointwise. In that case we
		define the \emph{pointwise distance residual}
		\[
		r(u;z)
		:=
		\dist^2\bigl((\mathcal{D}u)(z),\,(\mathcal{F}u)(z)\bigr),
		\]
		...
		$\mathcal{L}$ is a Monte-Carlo discretization of the integral of
		$r(u_\theta;\cdot)$. 
	\end{definition}
	...
	\begin{proof}
		...
		\]
		Moreover, $t\mapsto w_n(t)$ is (Lebesgue) measurable: by \textup{(H1)}--\textup{(H2)} and the continuity of $x_n$, the multifunction $t\mapsto F(t,x_n(t))$ is measurable with compact convex values, and the metric projection of the measurable function $\dot{x}_n$ onto such a multifunction is measurable by the standard measurable-projection theorem for measurable, closed- and convex-valued multifunctions; see \cite[Theorem~8.2.11]{aubin_frankowska_2009}.
...
%% referencyjnego (patrz TODO powyzej).
...
Since the reference tube is a finite sample of trajectories, this quantity
is a \emph{one-sided discrepancy from the sampled reference cloud} rather
than a certificate of admissibility: a small value indicates that the
ensemble trajectories lie close to empirically generated admissible
trajectories, regardless of the random initialization.
...
verbatim. All quantitative results reported below therefore refer to
$\Phi_\varepsilon$.
%% TODO [Rev.]: alternatywnie ustawic eps=0 w kodzie i powtorzyc trening,
%% wtedy powyzszy akapit mozna usunac; ewentualnie dodac test wrazliwosci
%% na eps.
...
illustrates how the residual can instead be eliminated at the evaluation
points by construction, freeing the remaining degrees of freedom to
