...
\label{sec:numerics}
%% ============================================================
To asses the numerical performance of the DR-PINNs approach proposed in the preceding sections,
...
%% ------------------------------------------------------------
\textcolor{red}{TO BE COMPLETED BY KRIS}
%% ------------------------------------------------------------
...
\begin{itemize}
	...
	$\mathbb{R}\setminus\{0\}$, and at $s=0$, $\Phi(s_n)\subset\{-\lambda,\lambda\}\subset[-\lambda,\lambda]=\Phi(0)$
...
\begin{remark}
	...
	\cite{Brezis1973}, since $-\Delta-\lambda\,\partial|\cdot|$ is a sum of
...
The DR-PINN loss for this example is~\eqref{eq:pde-distance-loss} with
$\Phi$ as in \eqref{eq:relay-Phi}. Since $\Phi(s)$ is an interval of
...
\begin{figure}[htbp]
  ...
  \caption{Reference solution: $\|u(t)\|_{L^2(\Omega)}$ on a log scale for
    ...
    curve does not extinguish within $[0,T]$.}
...
\begin{figure}[htbp]
  ...
  \caption{Validation of the DR-PINN against the reference solution
    ($\lambda=0.6$). Left: $L^2(\Omega)$-norm decay on a log scale; the
...
\begin{figure}[htbp]
  ...
  \caption{Extinction-time comparison across the primary sweep
    ...
    $0.6$, green: $0.8$). After extinction the reference drops to machine zero
